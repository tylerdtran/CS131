...
class Interpreter(InterpreterBase):
    ...
    def run(self, program):
        ...
        if main_func is None:
            super().error(ErrorType.NAME_ERROR, f"Function {main_func} not found")
    ...
    def __do_mcall(self, call_ast):
        ...
        # Map this keyword to object in mcall 
        # Perhaps use the self.env.set function to set this to the object in mcall
        
        act_obj = self.env.get(call_ast.get("objref"))
        if act_obj is None:
            super().error(ErrorType.NAME_ERROR, f"Variable {call_ast.get('objref')} not found")
        if isinstance(act_obj, Value):
            if act_obj.type() != Type.OBJECT:
                super().error(
                    ErrorType.TYPE_ERROR,
                    f"Method {call_ast.get('name')} is changed to non-function type.",
                )


        func_name = call_ast.get("name")

        closure_obj = act_obj.value().get_field(func_name)

        if closure_obj is None:
            super().error(ErrorType.NAME_ERROR, f"Method {func_name} not found in object")
        
        ## this affects Incorrectness | test_proto2 and test_obj5 but solves the Incorrectness | test_obj7 Incorrectness | test_proto_same_name_different_type_child
        if closure_obj.type() != Type.CLOSURE:
            super().error(ErrorType.TYPE_ERROR, f"Function {func_name} is changed to non-function type.")
        target_closure = closure_obj.value()
        # print(target_closure.value(), "DAMN")
        ...
        # self.env.set("this", act_obj)
        new_env["this"] = act_obj
    ...
    def __call_func(self, call_ast):
        ...
        if call_ast.elem_type == Interpreter.MCALL_DEF:
            ...
            if call_ast.get("name") not in act_obj.value().fields:
                super().error(ErrorType.NAME_ERROR, f"Method {call_ast.get('name')} not found in object")
        # obj_ast = mcall_ast.__eval_name("objref")
            target_closure = act_obj.value().fields.get(call_ast.get("name")).value()
            # print(target_closure.value(), "DAMN")
            actual_args = call_ast.get("args")
        else:
            actual_args = call_ast.get("args")
    ...
    def __prepare_env_with_closed_variables(self, target_closure, temp_env):
        # changed it so that it iterates through the items and not just the captured_env
    ...
    def __assign(self, assign_ast):
        ...
        if '.' in var_name:
            ...
            if obj.type() != Type.OBJECT:
                super().error(ErrorType.TYPE_ERROR, f"Variable {obj_name} not found")
            
            if field_name == "proto":
                parent_obj = self.__eval_expr(assign_ast.get("expression"))
                if parent_obj is None: 
                    super().error(ErrorType.NAME_ERROR, f"Prototype cannot be none")
                    
                if parent_obj.type() != Type.OBJECT and parent_obj.type() != Type.NIL:
                    super().error(ErrorType.TYPE_ERROR, f"Prototype must be an object")
                obj.value().proto = parent_obj

            value = self.__eval_expr(assign_ast.get("expression"))
            # obj.value().fields[field_name] = value
            obj.value().set_field(field_name, value) 
        else:
            # src_value_obj = copy.copy(self.__eval_expr(assign_ast.get("expression")))
            # left_val = self.__eval_expr(assign_ast.get("expression"))
            # if left_val.type() == Type.OBJECT:
            #     # print(left_val.value().fields)
            #     src_value_obj = copy.deepcopy(left_val)
            #     print(src_value_obj.value())
            # else:
            #     src_value_obj = copy.copy(left_val)
            src_value_obj = copy.copy(self.__eval_expr(assign_ast.get("expression")))
    ...
    def __eval_expr(self, expr_ast):
        ...
        if expr_ast.elem_type == Interpreter.OBJ_DEF:
            return Value(Type.OBJECT, Object()) # instantiate an empty object for the time being
    ...
    def __eval_name(self, name_ast):
        ...
        if '.' in var_name:
            ...
            retrieved_obj = obj.value().get_field(field_name)
            if retrieved_obj is None:
                super().error(ErrorType.NAME_ERROR, f"Field {field_name} not found in object")
            # return obj.value().get_field(field_name)
            else: 
    ...
    # obj_name, field_name = "example.com".split(".") # need to split the name of the object and the field, so then we can assign the object to the field
...
def main():
    # PROTO: Have the object point to proto object, that way fields can be inherited and manipulated and still reflectant 
    # While loop?
    ...
    #     while (person1.x > 0){
    #     print(person1.x);
    #     person1.x = person1.x - 1; 
    #     }
    #     print(person1.x);
    #     }"""
    # not passing this testcase
    # program_source = """func main() {
    #                     p = @;    
    #                     print(p.proto);
    #                 }
    ...
    # program_source = """func main() {
    #                         x = @;
    #                         x.set = lambda(x) { this.ddd = x; };
    #                         x.f = lambda() { this.myself = this; };
    #                         x.g = lambda() { me = this.myself; print(me.ddd); print(this.ddd); me.set(5); print(me.ddd); print(this.ddd); };
    ...
    #                                 }
    #                                 """
    # I was failing test_proto_while_loop previously and I found an issue in my code where I wasn't handling references 
    # for functions / closures / objects correctly. I was treating them as references rather than object references, so when 
    # I did cur_obj = a and then cur_obj = new_obj, it was overwriting a to be a blank object, so it was producing a name error.
